...
namespace Amazon.Web.Areas.Customer.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Coupun(string coupunName)
        {
            ...
            // If you want to store CoupunType as a string, convert it to string
            CartVM.orderHeader.CoupunCode = coupun.Name;
            CartVM.orderHeader.CoupunDiscount = coupun.Discount;
            CartVM.orderHeader.CoupunType = coupun.Type.ToString();  // Convert enum to string here

            // Correct enum comparison
            if (coupun.Type == CoupunType.Percent)
            {
                CartVM.orderHeader.OrderTotal = CartVM.orderHeader.SubTotal -
                    (CartVM.orderHeader.SubTotal * (CartVM.orderHeader.CoupunDiscount / 100));
            }
            else if (coupun.Type == CoupunType.Currency)
            {
                CartVM.orderHeader.OrderTotal = (CartVM.orderHeader.SubTotal - CartVM.orderHeader.CoupunDiscount);
            }

            TempData["success"] = "Coupun Added Successfully";
            byte[] c = Encoding.ASCII.GetBytes(coupunName);
            await HttpContext.Response.Body.WriteAsync(c);
            return RedirectToAction("Summary");
        }
